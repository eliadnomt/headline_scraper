...
def get_articles_for_all(topics: List[str]) -> List[TopicList]:
    """
    Runs the scraper function for all search topics listed in src/secret_config.py.
    :param topics: a list of topics from src/secret_config.py
...
def get_articles_for_specific(topic: str) -> TopicList:
    ...
    Uses the feedparser library to scrape the RSS entries for Google
            searches using the topic keywords in src/secret_config.py.
    ...
    logger.info(
        f"Retrieved RSS data for {len(article_list)} articles for topic {topic.upper()}"
    )
